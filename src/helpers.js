...
export const applyPattern = ({ pattern, state, initialX, initialY }) => {
  ...
  const offsetX = Math.floor(pattern.length / 2);
  const offsetY = Math.floor(pattern[0].length / 2);

  pattern.forEach((column, x) =>
    column.forEach(
      (cell, y) =>
        (newState[(initialX - offsetX + x + columnsNumber) % columnsNumber][
          (initialY - offsetY + y + rowsNumber) % rowsNumber
        ] = cell),
    ),
  );
  return newState;
